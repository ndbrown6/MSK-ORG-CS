...
smry_ = manifest %>%
	reshape2::melt(id.vars = c("sample_name", "is_tp53_mutated_0_1", "carcinoma_classification", "sarcoma_classification", "sarcoma_predominant_yes_no", "her2_ihc"),
...
smry_ = manifest %>%
	reshape2::melt(id.vars = c("sample_name", "is_tp53_mutated_0_1", "carcinoma_classification", "sarcoma_classification", "sarcoma_predominant_yes_no", "trop2_overall_ihc"),
	...
	dplyr::summarize(trop2 = mean(value, na.rm = TRUE),
			 ...
			 sarcoma_classification = unique(sarcoma_classification),
			 sarcoma_predominant = unique(sarcoma_predominant_yes_no),
			 trop2_ihc = mean(trop2_overall_ihc, na.rm=TRUE)) %>%
	dplyr::ungroup() %>%
	dplyr::mutate(sample_name = gsub(pattern = "cs", replacement = "CS", x = sample_name, fixed = TRUE)) %>%
	...
	dplyr::mutate(tp53_mutated = case_when(
		is.na(tp53_mutated) ~ 5,
		TRUE ~ tp53_mutated
	)) %>%
	dplyr::mutate(sarcoma_classification = case_when(
		is.na(sarcoma_classification) ~ 5,
		TRUE ~ sarcoma_classification
	))
...
draw(Heatmap(matrix = smry_ %>%
	     	      ...
	     	      tibble::column_to_rownames(var = "sample_name") %>%
	     	      t(),
	     col = c("1" = "#fb9a99", "2" = "#1f78b4", "3" = "#b2df8a", "4" = "#33a02c", "5" = "#dedbd8", palette),
	     name = " ",
	     na_col = "#f0f0f0",
	     rect_gp = gpar(col = "white"),
	     border = NA,
...
pdf(file = "../res/Trop2_Strip_Chart_Sorted.pdf", width = 14, height = 6)
draw(Heatmap(matrix = smry_ %>%
	     	      dplyr::select(-trop2) %>%
	     	      dplyr::select(-trop2_ihc) %>%
	     	      as.data.frame() %>%
